...
def get_steamcharts_app_ids(pages_to_scrape=200):
    print(f"--- Step 1: Fetching game App IDs from {pages_to_scrape} pages of SteamCharts ---")
    app_ids = set()
    ...
    for page_num in range(1, pages_to_scrape + 1):
        ...
        try:
            ...
            game_rows = soup.select("#top-games tbody tr")
            ...
            for row in game_rows:
                ...
                if link_tag:
                    ...
                    if app_id_match:
                        app_ids.add(app_id_match.group(1))
            
            print(f"Collected {len(app_ids)} unique App IDs... (Page {page_num}/{pages_to_scrape})")
            time.sleep(0.5)

        except requests.RequestException as e:
            print(f"Error fetching SteamCharts page {page_num}: {e}. Retrying in 10 seconds...")
            time.sleep(10)
            page_num -= 1
    ...
    print(f"Successfully found {len(app_ids)} unique App IDs from SteamCharts.")
    return list(app_ids)
...
if __name__ == "__main__":
    PAGES_TO_SCRAPE = 200
    GAME_COUNT = PAGES_TO_SCRAPE * 25
    OUTPUT_FILENAME = f'steam_top_{GAME_COUNT}_games_from_steamcharts.csv'
    
    all_app_ids = get_steamcharts_app_ids(pages_to_scrape=PAGES_TO_SCRAPE)
    
    if not all_app_ids:
        print("Could not retrieve any App IDs. Exiting.")
    else:
        processed_ids = load_processed_ids(OUTPUT_FILENAME)
        if processed_ids:
            print(f"\nFound {len(processed_ids)} already scraped games. Resuming...")
            
        remaining_ids = [app_id for app_id in all_app_ids if app_id not in processed_ids]
        
        print(f"Total IDs to process: {len(remaining_ids)}")

        HEADERS_CSV = [
            'App ID', 'Name', 'Price (USD)', 'Is Free', 'Release Date', 
            'Days Since Release', 'Tags', 'Controller Support', 'Steam Deck Support', 
            'Languages', 'Review Summary', 'Total Reviews', 'Positive Reviews',
            'All-Time Peak Players', 'Avg Players (30 Days)'
        ]
        ...
        with open(OUTPUT_FILENAME, 'a', newline='', encoding='utf-8') as csvfile:
            ...
            for i, app_id in enumerate(remaining_ids):
                print(f"\n--- Scraping game {i+1}/{len(remaining_ids)} (Overall: {len(processed_ids) + i + 1}/{len(all_app_ids)}) (App ID: {app_id}) ---")
                data = scrape_game_data(app_id)
                
                if data:
                    writer.writerow(data)
                    csvfile.flush()
                    print(f"Successfully scraped and saved: {data.get('Name', 'N/A')}")
                else:
                    print(f"Skipping App ID: {app_id} due to errors.")
